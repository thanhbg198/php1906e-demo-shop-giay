...
CREATE ALGORITHM=UNDEFINED DEFINER=`demo_shop_giay1`@`localhost` SQL SECURITY DEFINER VIEW `bangso`  AS  select ((((((`number1`.`so` * 10) * 10) * 10) + ((`number2`.`so` * 10) * 10)) + (`number3`.`so` * 10)) + `number4`.`so`) AS `so` from (((`number` `number1` join `number` `number2`) join `number` `number3`) join `number` `number4`) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`demo_shop_giay1`@`localhost` SQL SECURITY DEFINER VIEW `dates`  AS  select ('2010-01-01' + interval `bangso`.`so` day) AS `date` from `bangso` ;
